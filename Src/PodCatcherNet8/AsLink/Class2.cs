...
namespace WebScrap
{
  public partial class WebScraperBase
  {
    ...
    protected static ICredentials getICredentials(Uri uri) =>
      ...
      CredentialCache.DefaultNetworkCredentials; //TU: 2/2 //note: is not required for getiing pics and files - html only (3-jun-2010).//const int s123 = 345; const string sABC = "are you crazy!"; // from old <-- 4+4 <--
  ...
  Console.WriteLine("\nThe HttpHeaders are \n{0}",myWebRequest.Headers);
    ...
    public static string GetCachedFileNameFromUrl(string url)
    {
      ...
      var folder = Path.Combine(OneDrive.VpdbFolder, fn.Split('-')[0]);
      //#if CS8
    ...
    public static string GetCachedFileNameFromUrl_(string url, bool downloadIfFileNotExists)
    {
      ...
      {
        _ = SaveWebFileToLocalFile(url, fn); // throw new Exception("Nust simplify to remove SaveWebImageToFile()"); // 
      }
