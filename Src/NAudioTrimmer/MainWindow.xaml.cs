...
using NAudioTrimmer.Properties;
using System;
...
namespace NAudioTrimmer
{
  public partial class MainWindow : Window
  {
    ...
    void slS_PreviewKeyDown(object sender, KeyEventArgs e)
    {
      //            e.Handled = true;
    }
    ...
    {
      hlPath.NavigateUri = new Uri(tbPath.Text = $"{Path.GetDirectoryName(me1.Source.LocalPath)}\\");
      tbFile.Text = $"{Path.GetFileName(me1.Source.LocalPath)}";
    ...
    async void onDrop(object s, DragEventArgs e)
    {
      ...
      Settings.Default.LastFile = files.First();
    ...
    async Task loadShowWaveForm()
    {
      ...
      im1.Source = null;
      await Task.Run(() => _nauHelper.LoadWaveFormToBitmap(Settings.Default.LastFile)).ContinueWith(_ =>
      {
