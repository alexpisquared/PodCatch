...
public class AdvertCutter
{
  ...
  public static void CreateSummaryAnnons(double ttlDurationMin, string trgDir)
  {
    try
    {
      ...
      _ = CreateWavConvertToMp3File($@"{trgDir}\0000 BeginningEndOfAllShows {DateTime.Now:MMdd-HHmm}  .wav.mp3", msg);
    }
    catch (Exception ex) { _ = ex.Log(); }
  }

  public static void CreateOverwriteAnnons(string feedName, DateTime pubDate, string castTitle, int castsLeft, double durationLeftMin, double durnMin, string srcFile) // used 
  {
    if (srcFile.EndsWith(MediaHelper.AnnonsExt)) return;

    var annonsFile = CalcAnnonsFilename(srcFile);

    if (File.Exists(annonsFile)) File.Delete(annonsFile);
    ...
    try
    {
      var dn = (durnMin == ConstHelper.Unknown4004Duration) ? "unknown " : (
        durnMin > 8 ? $"{durnMin:N0} minutes" :
        durnMin > 1.99 ? $"{durnMin:N1} minutes" :
        $"{durnMin * 60.0:N0} seconds");
      ...
      var textToSpeak =
        $"'{RemoveFirstCharacterIfFollowedBySpace(castTitle.Replace("BeatRunning", "").Replace("Speedmaster", "").Replace("Fitness Version", "").Replace(" min", " ").Replace(" sec", "."))}', ({dn}).";

      //if (castsLeft <= 5) textToSpeak += $" Followed by {castsLeft - 1} others.";

      _ = CreateWavConvertToMp3File(annonsFile, textToSpeak);
    }
    catch (Exception ex) { _ = ex.Log(); }
  }
  ...
  private static string RemoveFirstCharacterIfFollowedBySpace(string castTitle)
  {
    ...
    for (var i = 1; i <= 2; i++) // do not pronounce the ordering prefix, like: a 123... 1b 323... 001 Name...
    {
      if (castTitle[i] == ' ') return castTitle.Substring(i).Trim();
      if (castTitle[0] == '0' && castTitle[i + 1] == ' ') return castTitle.Substring(i + 1).Trim();
    }

    return castTitle;
  }

  public static string CalcAnnonsFilename(string annonsFile)
  {
    var nm = Path.GetFileNameWithoutExtension(annonsFile);
    var xt = Path.GetExtension(annonsFile);
    annonsFile = annonsFile.Replace(nm, nm + " ").Replace(xt, ".wav.mp3");
    return annonsFile;
  }
  ...
  public static void OffsetAndCutIntoPieces(string feedName, string mp3file, long oneMinuteSizeInBytes, long advOffset, string srcDir, string trgDir, DateTime pubDate, string castTitle, int castsLeft, double ttlDurationMin, double durationLeftMin, double partSizeMin) // old
  {
    ...
    try
    {
      ...
      _ = org.Seek(advOffset, SeekOrigin.Begin);
      ...
      if (ttlDurationMin > 2)
        _ = CreateWavConvertToMp3File(mp3file.Replace(ext, "\\[000]---Intro---.wav").Replace(srcDir, trgDir),
      ...
      for (var pieceNum = 0; pieceNum < pieceCount - 0; pieceNum++) // 
      {
                ...
                (pieceCount - pieceNum == 1 / partSizeMin))            // last 1.
        {
          ...
          if (!File.Exists(wavNum))
            _ = CreateWavConvertToMp3File(wavNum, pieceCount - pieceNum == 1 / partSizeMin ? "This is the very Last minute" :
  ...
  public static string ReplaceOrgWithAdRemoved_OLD(string mp3file)
  {
    ...
    return offset > 0 ? cutOffFirstBytes(mp3file, offset) : null;
  }
  ...
  static string cutOffFirstBytes(string mp3file, long offset)
  {
    ...
    var org = File.Open(mp3file, FileMode.Open, FileAccess.Read, FileShare.Read); // new FileStream(fi, FileMode.Open, FileAccess.Read);
    org.Position = offset;
    _ = org.Seek(offset, SeekOrigin.Begin);
    ...
    _ = org.Read(biteContentOfTheFile, 0, biteContentOfTheFile.Length);
    org.Close();
