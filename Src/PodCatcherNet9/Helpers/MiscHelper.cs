...
namespace PodCatcherNet9.Helpers
{
  public class MiscHelper // NEW
  {
    public static string GetSmartPathFileName(string castUrl, DateTime fi, string prefix, string subdir, string title, bool? IsNewerFirst)
    {
      ...
      var pref = (string.IsNullOrWhiteSpace(prefix) ? "" : prefix) + (IsAudio(fnEx) && IsNewerFirst == true ? GetReverseTimeOrderPrefix(fi) : ""); // only for audio && !subfoldered
      var spfn = string.IsNullOrWhiteSpace(subdir) ? pref + fnEx : Path.Combine(subdir, pref + fnEx);
 ...
 _ignoreList = @"C:\0\0\WebScrape\PodcastClient\IgnoreList.txt";
    ...
    public static DateTime GetDateFromAnyFormatString(string str)
    {
      ...
      if (!string.IsNullOrEmpty(strDate) && DateTime.TryParse(strDate, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out dt))
        return dt;
    ...
    {
      var podFileName = Path.GetFileNameWithoutExtension(castUrl).Replace("%20", " ");
      var podFileExtn = Path.GetExtension(castUrl).Replace("%20", " ");
