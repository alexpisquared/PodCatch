...
using MVVM.Common;
using PodCatcherNet9.Properties;
...
public partial class AsyncFineTuningVM : BindableBaseViewModel
{
  ...
  protected override async Task AutoExecAsync()
  {
    ...
    WinTitle = $"PodCatcherNet9 - {_crlnTS:ddd HH:mm}";
  ...
  ICommand _DbChck;               /**/ public ICommand DbChckCmd => _DbChck ??= new RelayCommand(onDbChck, x => canDbChck) { GestureKey = Key.A, GestureModifier = ModifierKeys.Control };
  ICommand _DbSave;               /**/ public ICommand DbSaveCmd => _DbSave ??= new RelayCommand(async x => await onDbSaveAsync(x), x => canDbSave) { GestureKey = Key.B, GestureModifier = ModifierKeys.Control };
  ICommand _SeeDLs;               /**/ public ICommand SeeDLsCmd => _SeeDLs ??= new RelayCommand(onSeeDLs, x => canSeeDLs) { GestureKey = Key.C, GestureModifier = ModifierKeys.Control };
  ICommand _Fd;                   /**/ public ICommand FdCmd => _Fd ??= new RelayCommand(onFd, x => canFd) { GestureKey = Key.D, GestureModifier = ModifierKeys.Control };
  ICommand _Recent;               /**/ public ICommand RecentCmd => _Recent ??= new RelayCommand(onRecent, x => canRecent) { GestureKey = Key.E, GestureModifier = ModifierKeys.Control };
  ICommand _Dnlded;               /**/ public ICommand DnldedCmd => _Dnlded ??= new RelayCommand(onDnlded, x => canDnlded) { GestureKey = Key.E, GestureModifier = ModifierKeys.Control };
  ICommand _Pendng;               /**/ public ICommand PendngCmd => _Pendng ??= new RelayCommand(onPendng, x => canPendng) { GestureKey = Key.E, GestureModifier = ModifierKeys.Control };
  ...
  void onCancelAsy(object p) { Bpr.Beep1of2(); _cts?.Cancel(); Bpr.Beep2of2(); }
  ...
  void onStopCntDn(object p)
  {
    ...
    {
      if (!_db.ChangeTracker.Entries().Any(e => e.State is EntityState.Added or EntityState.Modified or EntityState.Deleted))
  ...
  void onDelCasts(object p)
  {
    ...
    _ = _db.DnLds.RemoveRange(remove);
    DnLdList.ClearAddRangeAuto(SelectFeed.DnLds);
    info();
  }
  void onDelCasAl(object p) { Bpr.BeepClk(); _ = _db.DnLds.RemoveRange(SelectFeed.DnLds); DnLdList.Clear(); info(); }
  void onF6(object p)
  {
  ...
  void addNewFeed(Feed feed)
  {
    if (!_db.Machines.Any(r => string.Compare(r.Id, Environment.MachineName, true) == 0))
      _ = _db.Machines.Add(new Machine { Id = Environment.MachineName, IsUsable = true, TargetDrive = "C", Note = $"Auto added on {DateTime.Now}" });
  ...
  void addNewFeed(string url, string name, string note, bool isActive, DateTime Since)
  {
    ...
    if (!_db.Machines.Any(r => string.Compare(r.Id, Environment.MachineName, true) == 0))
      _ = _db.Machines.Add(new Machine
      {
  ...
  static Feed preCreateNewFeed(DateTime now) => new()
  {
    //Url = url,
    //Name = name,
    //Note = note,
  ...
  static Feed preCreateNewFeed(string url, string name, string note, bool isActive, DateTime Since, DateTime now) => new()
  {
    Url = url,
    Name = name,
    Note = note,
  ...
  void threadAwareRun(Action actn)
  {
    ...
    if (Application.Current.Dispatcher.CheckAccess()) // if on UI thread
      ...
      _ = Application.Current.Dispatcher.BeginInvoke(DispatcherPriority.Normal, new Action(() => actn()));
